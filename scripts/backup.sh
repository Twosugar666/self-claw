#!/bin/bash
# XiaoWe 数据定期备份脚本
# 用法: crontab -e → 0 4 * * * /opt/xiaowe-all/scripts/backup.sh
#
# 功能:
...
# ---- 配置 ----
PROJECT_DIR="${KARVIS_PROJECT_DIR:-/root/XiaoWe}"
...
BACKUP_FILE="${BACKUP_DIR}/xiaowe-data-${TIMESTAMP}.tar.gz"
...
while IFS= read -r old_file; do
    rm -f "$old_file"
    DELETED=$((DELETED + 1))
done < <(find "$BACKUP_DIR" -name "xiaowe-data-*.tar.gz" -mtime +${KEEP_DAYS} -type f 2>/dev/null)
...
# ---- 统计 ----
TOTAL_BACKUPS=$(find "$BACKUP_DIR" -name "xiaowe-data-*.tar.gz" -type f | wc -l | tr -d ' ')
