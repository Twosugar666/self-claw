...
# ============================================================
#  XiaoWe 一键安装脚本
#  用法: git clone ... && cd XiaoWe && ./setup.sh
# ============================================================
...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║      XiaoWe 安装向导 (多用户版)         ║${NC}"
...
if [ -n "$PUBLIC_IP" ]; then
    ...
    echo -e "  ${YELLOW}  （不配会导致 XiaoWe 无法发送消息）${NC}"
else
...
# 询问是否立即启动
read -p "是否立即启动 XiaoWe? (y/N): " START_NOW
if [[ "$START_NOW" == "y" || "$START_NOW" == "Y" ]]; then
    echo ""

    # 启动 XiaoWe
    echo -e "${GREEN}启动 XiaoWe...${NC}"
    # 如果有虚拟环境，确保已激活
    ...
    # 检查 XiaoWe 是否启动成功
    if ! kill -0 $XIAOWE_PID 2>/dev/null; then
        echo -e "${RED}XiaoWe 启动失败，请检查日志${NC}"
        exit 1
    fi
    echo -e "  ${GREEN}✓ XiaoWe 已启动 (PID: $XIAOWE_PID)${NC}"
    ...
    if [ "$TUNNEL_CMD" == "cloudflared" ]; then
        ...
        echo ""
        TUNNEL_LOG=$(mktemp /tmp/xiaowe_tunnel_XXXXXX.log)
        ...
        if [ -n "$TUNNEL_URL" ]; then
            ...
            # 重启 XiaoWe 使新配置生效
            kill $XIAOWE_PID 2>/dev/null
            sleep 1
            $PYTHON_CMD app.py &
            XIAOWE_PID=$!
            sleep 2
            echo -e "  ${GREEN}✓ XiaoWe 已重启（加载新配置）${NC}"
        ...
        trap "kill $XIAOWE_PID $TUNNEL_PID 2>/dev/null; rm -f '$TUNNEL_LOG'; exit 0" INT TERM
        wait $XIAOWE_PID
        ...
        ngrok http 9000 &
        TUNNEL_PID=$!
        trap "kill $XIAOWE_PID $TUNNEL_PID 2>/dev/null; exit 0" INT TERM
        wait $XIAOWE_PID
    else
        echo ""
        echo -e "${YELLOW}没有内网穿透工具，XiaoWe 仅在本地可用 (http://localhost:9000)${NC}"
