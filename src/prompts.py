...
SKILL_PROMPT_LINES = {
    ...
    "settings.ai_name": '**settings.ai_name** `{ai_name}` — 给 AI 起昵称（用户说"我叫你XX"、"叫你XX"、"你叫XX"时触发。这是用户给 XiaoWe 起的名字）',
...
- **⚠️ 核心规则：用户说"笔记""日记"时，默认数据源是 `03-WeChat`（微信离线数据目录），而非 02-Notes。** 因为用户的笔记/日记主要存储在微信导出的 03-WeChat 目录中。只有明确说"速记""工作笔记""情感日记"等 XiaoWe 系统笔记时，才去 02-Notes。
...
FLASH_REPLY = """你是 XiaoWe 的回复生成模块。根据以下信息生成给用户的最终回复。
...
REFLECT_RESPONSE = """你是用户的 AI 伴侣 XiaoWe。用户刚回答了一个深度自问。请给出一个温柔、有洞察力的回应。
